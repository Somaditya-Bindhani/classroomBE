...
const addSubjects = async (req, res, next) => {
  ...
  if (isValidSubject) {
    return next(
      new HttpError("Subject exists.Try adding different subject", 406)
    );
  }
...
const addAttendance = async (req, res, next) => {
  ...
  existingUser.subjects[foundSubjectIndex].attendance = [
    ...attendanceList,
    newAttendance,
  ];
...
const deleteAttendance = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Selected Attendance could not be deleted.Try Again", 500)
    );
  }
  const helperData = helper(existingUser.subjects);
...
const updateAttendance = async (req, res, next) => {
  ...
  if (attendanceToBeUpdatedIndex === -1) {
    return next(
      new HttpError("Attendance not found,so could not update attendance", 406)
    );
  }

  existingUser.subjects[foundSubjectIndex].attendance[
    attendanceToBeUpdatedIndex
  ].attendanceType = attendanceType;
  ...
  try {
    ...
    return next(
      new HttpError("Could not update selected attendance,try again later", 500)
    );
  }
  const helperData = helper(existingUser.subjects);
