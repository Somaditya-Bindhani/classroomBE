...
const signup = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return next(
      new HttpError("Invalid Inputs.Please fill the form correctly ", 501)
    );
  }

  if (req.body.password !== req.body.confirmPassword) {
    return next(
      new HttpError("Password and Confirm Password does not match", 502)
    );
  }
...
const login = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return next(
      new HttpError("Invalid Inputs.Please fill the form correctly ", 500)
    );
  }
